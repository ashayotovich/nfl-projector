...
class PDF(FPDF):
    def header(self):
        ...
        # Title
        self.cell(210, 0, f'NFL Week {week} Gambling Report', 0, 0, 'C')
        # Line break
        self.ln(30)
    ...
    def footer(self):
        ...
        # Page number
        self.cell(0, 10, 'Page ' + str(self.page_no()) + ' / 14', 0, 0, 'C')
...
fig, ax = plt.subplots()
print(len(game_book))
for game in range(len(game_book)):
    ...
    while i <= j:
        ...
        if home_delta > (-1 * home_line):
            ...
            ax.scatter(home_delta, total, c=away_color2)
        else:
    ...
    plt.plot()
    plt.draw()
    plt.savefig(f"week{week}/figures/{away_name}_at_{home_name}_scatter")
    plt.cla()
    ...
    plt.ylim((-1, 60))
    plt.legend()
    plt.savefig(f"week{week}/figures/{away_name}_at_{home_name}_scores")
    ...
    plt.savefig(f"week{week}/figures/{away_name}_at_{home_name}_totals")
    ...
    fig = plt.gcf()
    fig.gca().add_artist(centre_circle)
    plt.savefig(f"week{week}/figures/{away_name}_at_{home_name}_spread")
    ...
    a_cover_pct = a_cover / (a_cover + b_cover + no_cover)
    b_cover_pct = b_cover / (a_cover + b_cover + no_cover)
    no_cover_pct = no_cover / (a_cover + b_cover + no_cover)

    if a_cover_pct > 0.5:
        ats_pick = home_name
        ats_value = int(100 * a_cover_pct)
        ats_line = home_line
    elif b_cover_pct > 0.5:
        ats_pick = away_name
        ats_value =int(100 * b_cover_pct)
    ...
    pdf.image(f'week{week}/figures/{away_name}_at_{home_name}_scatter.png', 15, 115, 90)
    pdf.image(f'week{week}/figures/{away_name}_at_{home_name}_scores.png', 115, 115, 90)
    pdf.image(f'week{week}/figures/{away_name}_at_{home_name}_spread.png', 15, 200, 90)
    pdf.image(f'week{week}/figures/{away_name}_at_{home_name}_totals.png', 115, 200, 90)

pdf.output(f"week{week}/week{week}_report.pdf", 'F')
